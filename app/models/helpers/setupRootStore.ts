...
export async function setupRootStore(rootStore: RootStore) {
  ...
  try {
    ...
    // Verify restoredState has the expected structure before applying
    if (restoredState && typeof restoredState === 'object') {
      try {
        applySnapshot(rootStore, restoredState);
      } catch (applyError) {
        console.error('Error applying snapshot:', applyError);
        // Instead of failing, we'll continue with the default state
      }
    } else {
      console.log('Restored state was not valid, using default state');
    }
  } catch (e) {
